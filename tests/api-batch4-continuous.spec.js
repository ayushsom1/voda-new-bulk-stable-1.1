...
test('API Batch 4 Continuous Automation', async () => {
  console.log('🚀 Starting Batch 4 continuous automation...\n');
  
  // Test configuration
  const DURATION_MINUTES = 2;
  const DURATION_MS = DURATION_MINUTES * 60 * 1000;
  const username = 'Del353190x1';
  const password = 'Shivam@123456';
  
  // Batch file for this spec
  const batchFile = './data/phone_number_batch4.txt';
  ...
  try {
    ...
    while (Date.now() - startTime < DURATION_MS) {
      ...
      console.log(`\n${'='.repeat(80)}`);
      console.log(`🔄 BATCH 4 - CYCLE ${cycleCount} - ${remainingTime} minutes remaining`);
      ...
      console.log(`📦 Processing Batch 4: ${batchFile}`);
      ...
      try {
        ...
        console.log(`   ❌ Batch 4 failed: ${batchError.message}`);
        batchResult.error = batchError.message;
        
      } finally {
        batchResult.duration = Date.now() - batchStartTime;
        console.log(`   ⏱️  Batch 4 completed in ${Math.round(batchResult.duration / 1000)}s`);
      }
      ...
      console.log(`✅ Batch 4 - Cycle ${cycleCount} completed - ${cycleResults.cycleStats.totalNumbers} numbers processed`);
    }
    ...
    console.log(`\n${'='.repeat(80)}`);
    console.log('🏁 BATCH 4 SESSION COMPLETED');
    console.log(`${'='.repeat(80)}`);
    console.log(`📅 Session ID: ${sessionId}`);
    console.log(`🕐 Start Time: ${new Date(startTime).toLocaleString()}`);
    ...
    allCycleResults.forEach(cycle => {
      cycle.batchResults.forEach(batch => {
        const batchReport = {
          testName: `API Batch 4 - Cycle ${cycle.cycleNumber}`,
        ...
        const tempFileName = `api-batch4-cycle${cycle.cycleNumber}-test-${sessionId}.json`;
    ...
    console.log(`📊 Batch 4 session completed - ${batchReports.length} result files saved for consolidated reporting`);
    ...
    console.log('\n✨ Batch 4 continuous automation completed successfully!');
  }
});