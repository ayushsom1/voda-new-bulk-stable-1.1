...
test('API Batch 6 Continuous Automation', async () => {
  console.log('🚀 Starting Batch 6 continuous automation...\n');
  ...
  const batchFile = './data/phone_number_batch6.txt';
  ...
  try {
    ...
    while (Date.now() - startTime < DURATION_MS) {
      ...
      console.log(`\n${'='.repeat(80)}`);
      console.log(`🔄 BATCH 6 - CYCLE ${cycleCount} - ${remainingTime} minutes remaining`);
      ...
      console.log(`📦 Processing Batch 6: ${batchFile}`);
      ...
      try {
        ...
        console.log(`   ❌ Batch 6 failed: ${batchError.message}`);
        batchResult.error = batchError.message;
        
      } finally {
        batchResult.duration = Date.now() - batchStartTime;
        console.log(`   ⏱️  Batch 6 completed in ${Math.round(batchResult.duration / 1000)}s`);
      }
      ...
      console.log(`✅ Batch 6 - Cycle ${cycleCount} completed - ${cycleResults.cycleStats.totalNumbers} numbers processed`);
    }
    ...
    console.log(`\n${'='.repeat(80)}`);
    console.log('🏁 BATCH 6 SESSION COMPLETED');
    console.log(`${'='.repeat(80)}`);
    console.log(`📅 Session ID: ${sessionId}`);
    console.log(`🕐 Start Time: ${new Date(startTime).toLocaleString()}`);
    ...
    allCycleResults.forEach(cycle => {
      cycle.batchResults.forEach(batch => {
        const batchReport = {
          testName: `API Batch 6 - Cycle ${cycle.cycleNumber}`,
        ...
        const tempFileName = `api-batch6-cycle${cycle.cycleNumber}-test-${sessionId}.json`;
    ...
    console.log(`📊 Batch 6 session completed - ${batchReports.length} result files saved for consolidated reporting`);
    ...
    console.log('\n✨ Batch 6 continuous automation completed successfully!');
  }
});